...
class Preferences {
  ...
  write(file, data) { fs.writeFileSync(file + '.tmp', JSON.stringify(data), { mode: 0o600 }); fs.renameSync(file + '.tmp', file); }
  read() { return { ...this.data, themes, secure: this.secure(), keys: Object.fromEntries(['openai','deepseek'].map(p => [p, !!this.sessionKeys[p] || !!this.encrypted[p]])) }; }
  save(input) {
    ...
    this.data = { ...this.data, theme: input.theme, custom, dim, gradient, reduceMotion: !!input.reduceMotion };
